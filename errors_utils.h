

#ifndef MAMN14_ERRORS_UTILS_H
#define MAMN14_ERRORS_UTILS_H


#define ERR_CODE_LABEL_ALREADY_IN_USE 1
#define ERR_CODE_LABEL_IS_INSTRUCTION 2
#define  ERR_CODE_LABEL_IS_OPERATION 3
#define ERR_CODE_LABEL_IS_REGISTER 4
#define ERR_CODE_INVALID_DATA_TYPE 5
#define ERR_CODE_INVALID_STRING_TYPE 6
#define ERR_CODE_TOO_MANY_ARGUMENTS 7
#define ERR_CODE_INVALID_ARGUMENTS 8
#define ERR_CODE_INVALID_OPERANDS 9
#define ERR_CODE_INVALID_OPERANDS_AMOUNT 10
#define ERR_CODE_INVALID_OPERANDS_SYNTAX 11
#define ERR_CODE_INVALID_OPERANDS_TYPE 12


struct HashTable* get_errors_map();




#endif //MAMN14_ERRORS_UTILS_H
